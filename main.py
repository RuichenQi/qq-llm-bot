...
def _extract_quoted_text(payload: Dict[str, Any]) -> Optional[str]:
    """Pull a plain-text snippet out of a get_msg response."""
...
async def amain() -> int:
    ...
    async def fetch_reply_text(msg_id: str) -> Optional[str]:
        if client is None:
            return None
        payload = await client.get_msg(msg_id)
        if payload is None:
            return None
        return _extract_quoted_text(payload)
    ...
    handler = Handler(
        ...
        fetch_reply_text=fetch_reply_text,
